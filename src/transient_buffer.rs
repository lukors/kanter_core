use std::{collections::VecDeque, fs::File, mem::size_of, io::{Read, Seek, SeekFrom, Write}, sync::{Arc, RwLock, atomic::{AtomicBool, Ordering}}};
use tempfile::tempfile;

use crate::{error::{Result, TexProError}, slot_data::{Buffer, ChannelPixel, Size}};
...
impl TransientBuffer {
    ...
    /// Makes sure the `TransientBuffer` is in memory and returns itself.
    ...
    pub fn to_memory(&mut self) -> Result<()> {
        if let Self::Storage(file, size) = self {
            ...
            *self = Self::Memory(Box::new(Buffer::from_raw(size.width, size.height, buffer_f32).ok_or(TexProError::Generic)?));
        }
...
impl TransientBufferQueue {
    ...
    pub fn add_buffer(&mut self, transient_buffer_container: Arc<TransientBufferContainer>) -> Result<()> {
        if transient_buffer_container.transient_buffer.read()?.in_memory() {
